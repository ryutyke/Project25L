...
void ADSGun::ServerRPC_PrimaryAttack_Implementation(const FVector& StartLoc, const FVector& TargetLoc, const float& SkillDamage) const
{
	...
	if (bResult)
	{
		...
		if (IsValid(TargetCharacter))
		{
			...
			if (TargetCharacter->GetClass()->ImplementsInterface(UDamageableInterface::StaticClass()))
			{
				IDamageableInterface::Execute_ReceiveDamage(TargetCharacter, SkillDamage, DamageEvent, Character->GetController(), Character);
			}
		}
